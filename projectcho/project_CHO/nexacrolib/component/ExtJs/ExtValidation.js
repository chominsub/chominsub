...
* @return N/A
*/  
pForm.ufnClearValidation = function(obj)
{
...
* @return N/A
*/  
pForm.ufnRemoveValidation = function(obj, sColumID)
{
	var newvalidObj = new Array();
	
	if (!this.ufnIsNull(obj.validationRuleSet))
	{
...
* @return N/A
*/
pForm.ufnSetValidation = function(obj, sColumID, sColumName, sValidRule)
{
...
pForm.ufnValidation = function()
{
	...
	for(var m=0;m<arguments.length;m++)
	{
		...
		if (this.ufnTypeOf(arguments[m]) == "string")
		{
	...
	for (var m=0;m<targetlist.length;m++)
	{
		...
		if( !this.ufnIsNull(dataset.validationRuleSet) )
		{
		...
		for(var i=0;i<dataset.getRowCount();i++)
		{
			...
			for(var j=0;j<validlist.length;j++)
			{
				...
				if (nullcheck == true)
				{
					if (this.ufnIsNull(columvalue))
					{
						// Validation ERROR class 사용 시
						if (this.useErrorClass) {
							this.ufnResetValidationCss(arguments);
						}
						...
						// 메시지 처리 : ${} 필수 입력 항목입니다.
						//this.ufnAlert("msg.err.validator", [this.ufnGetKoreanTarget(titile, "은(는)")+" 필수 입력 항목입니다."], null, "ufnValidationCallback");
						// 메시지 처리 : {0} 은(는) 필수 입력 항목입니다.
						this.ufnAlert("msg.err.validator.required", [titile], null, "ufnValidationCallback");
				...
				if (!this.ufnIsNull(columvalue))
				{
					...
					for(var k=0;k<checklist.length;k++)
					{
						...
						if (checkrule == "equalto" || checkrule == "fromto" || checkrule == "comparebig" || checkrule == "comparesmall")
						{
							...
							msg = this.ufnValidationCheckRule(columname, columvalue, checklist[k], compare1, compare2);
						}
						// 컬럼의 한개 값을 이용해서 처리하는 check 대상
						else {
							msg = this.ufnValidationCheckRule(columname, columvalue, checklist[k]);
						}
						
						// 에러시
						if (!this.ufnIsNull(msg))
						{
							// Validation ERROR class 사용 시
							if (this.useErrorClass) {
								this.ufnResetValidationCss(arguments);
							...
							// 메시지 처리 : ${}
							this.ufnAlert("msg.err.validator", [msg], null, "ufnValidationCallback");
...
*/
pForm.ufnValidationCheckRule = function(itemName, itemValue, validationRuleSet, compare1, compare2)
{
	var titile     = "";
	var columname1 = "";
	var columname2 = "";
	...
	case "length":
		...
		{
			//return this.ufnGetKoreanTarget(titile, "은(는)")+" "+arrItem2[1]+"자리 입니다.";
			// {0} 의 입력값은 {1} 자리이어야 합니다.
			return this.ufnGetMessage("msg.err.validator.length",[titile, arrItem2[1]]);
		}
		break;
	...
	case "rangelength":
		...
		{
			//return this.ufnGetKoreanTarget(titile, "은(는)")+" ("+arrItem2[1]+")자리에서 ("+arrItem2[2]+")자리로 등록해야 합니다.";
			// {0} 은(는) {1} 와(과) {2} 사이의 자리이어야 합니다.
			return this.ufnGetMessage("msg.err.validator.rangelength",[titile, arrItem2[1], arrItem2[2]]);
		}
		break;
	...
	case "maxlength":
		if (itemValue.length > parseInt(arrItem2[1]))
		{
			//return this.ufnGetKoreanTarget(titile, "은(는)")+" 최대 "+arrItem2[1]+"자리 입니다.";
			// {0} 의 입력값의 길이는 {1} 이하이어야 합니다.
			return this.ufnGetMessage("msg.err.validator.maxlength",[titile, arrItem2[1]]);
		}
		break;
	...
	case "minlength":
		if (itemValue.length < parseInt(arrItem2[1]))
		{
			//return this.ufnGetKoreanTarget(titile, "은(는)")+" 최소 "+arrItem2[1]+"자리 이상이어야 합니다.";
			// {0} 의 입력값의 길이는 {1} 이상이어야 합니다.
			return this.ufnGetMessage("msg.err.validator.minlength",[titile, arrItem2[1]]);
		}
		break;
		// 최대 size 크기(Byte) : maxlengthB:3
	case "maxlengthbyte":
		if (this.lookupFunc("ufnLengthByte").call(itemValue) > parseInt(arrItem2[1]))
		{
			//return this.ufnGetKoreanTarget(titile, "은(는)")+" 최대 "+arrItem2[1]+"자리 입니다.";
			// {0} 의 입력값의 길이는 {1} 이하이어야 합니다.
			return this.ufnGetMessage("msg.err.validator.maxlength",[titile, arrItem2[1]]);
	...
	case "minlengthbyte":
		...
		// 숫자 여부 : digits
	case "digits":		
		if (!this.lookupFunc("ufnIsDigit").call(itemValue))
		{
			//return this.ufnGetKoreanTarget(titile, "은(는)")+" 숫자만 입력 가능합니다.";
			// {0} 은(는) 숫자만 입력 가능합니다.
			return this.ufnGetMessage("msg.err.validator.digits",[titile]);
		}
		break;
		// 해당 숫자 이하 : min:7
	case "min":
		...
		{
			//return this.ufnGetKoreanTarget(titile, "은(는)")+" "+arrItem2[1]+" 이상의 숫자만 입력 가능합니다.";
			// {0} 은(는) {1} 이상의 숫자만 입력 가능합니다.
			return this.ufnGetMessage("msg.err.validator.min",[titile, arrItem2[1]]);
		}
		break;
		// 해당 숫자 이상 : max:7
	case "max":
		if (parseFloat(itemValue) > parseFloat(arrItem2[1]))
		{
			//return this.ufnGetKoreanTarget(titile, "은(는)")+" "+arrItem2[1]+" 이하의 숫자만 입력 가능합니다.";
			// {0} 은(는) {1} 이하의 숫자만 입력 가능합니다.
			return this.ufnGetMessage("msg.err.validator.max",[titile, arrItem2[1]]);
		}
		break;
	...
	case "declimit":
		...
		if (isExistDot == -1)
		{
			//return this.ufnGetKoreanTarget(titile, "은(는)")+" 소숫점 "+arrItem2[1]+" 자리로 구성되어야 합니다.";
			// {0} 은(는) 소숫점 {1} 자리로 구성되어야 합니다.
			return this.ufnGetMessage("msg.err.validator.declimit",[titile, arrItem2[1]]);
		}
		else
		{
		...
		// 날짜 년월일 체크 : date
	case "date":
		if (!this.lookupFunc("ufnIsYMD").call(itemValue))
		{
			//return titile + "에 등록된 날짜가 잘못되었습니다.";
			// {0} 은(는) 유효하지 않은 날짜 형식입니다.
			return this.ufnGetMessage("msg.err.validator.date",[titile]);
		}
		break;
		// 날짜 년월 체크 : dateym
	case "dateym":
		if (!this.lookupFunc("ufnIsYM").call(itemValue))
		{
			//return titile + "에 등록된 년월 형식의 날짜가 잘못되었습니다.";
			// {0} 은(는) 유효하지 않은 년월 형식입니다.
			return this.ufnGetMessage("msg.err.validator.dateym",[titile]);
		}
		break;
	...
	case "range":
		if (parseInt(itemValue) < parseInt(arrItem2[1]) || parseInt(itemValue) > parseInt(arrItem2[2]))
		{
			...
			return this.ufnGetMessage("msg.err.validator.range",[titile,arrItem2[1],arrItem2[2]]);
		}
		break;
	...
	case "code":
		...
		//return this.ufnGetKoreanTarget(titile, "은(는)")+" "+nexacro.replaceAll(validationRuleSet.split("code:")[1],":",",")+" 중 하나의 값이어야 합니다.";
		// {0} 은(는) {1} 중 하나의 값이어야 합니다.
		return this.ufnGetMessage("msg.err.validator.code",[titile,nexacro.replaceAll(validationRuleSet.split("code:")[1],":",",")]);
		break;
	...
	case "equalto":
		...
		{
			if (itemName.indexOf("^_^") != -1 ) {
				//return this.ufnGetKoreanTarget(columname1, "이(가)") + " "+columname2+"과 일치하지 않습니다.";
				// {0} 이(가) {1} 와(과) 일치하지 않습니다.
				return this.ufnGetMessage("msg.err.validator.equalto",[columname1,columname2]);
			}
			// 비교대상 칼럼의 title을 넘겨주지 않았을 경우 해당 값을 표시
			else {
				//return  titile + "의 값과 "+compare2+"와 일치하지 않습니다.";
				// {0} 이(가) {1} 와(과) 일치하지 않습니다.
				return this.ufnGetMessage("msg.err.validator.equalto",[titile,compare2]);
			}
		}
		break;
	...
	case "fromto":
		if (compare1 < compare2)
		{
			if( itemName.indexOf("^_^") > -1 )
			{
				...
				return this.ufnGetMessage("msg.err.validator.fromto",[columname1,columname2]);
			}
			// 비교대상 칼럼의 title을 넘겨주지 않았을 경우 해당 값을 표시
			else {
				//return  titile + "의 날짜가 "+compare2+"일 보다 작습니다.";
				// {0} 의 날짜가 {1} 의 날짜보다 작습니다.
				return this.ufnGetMessage("msg.err.validator.fromto",[titile,compare2]);
			}
		}
		break;
	...
	case "comparebig":
		if( parseFloat(compare1) < parseFloat(compare2) )
		{
			if( itemName.indexOf("^_^") != -1 ) {
				...
				// {0} 이(가) {1} 보다 작습니다.
				return this.ufnGetMessage("msg.err.validator.comparebig",[columname1,columname2]);
			}
			// 비교대상 칼럼의 title을 넘겨주지 않았을 경우 해당 값을 표시
			else {
				//return titile + "의 값은 "+compare2+"보다 커야 합니다.";
				// {0} 이(가) {1} 보다 작습니다.
				return this.ufnGetMessage("msg.err.validator.comparebig",[titile,compare2]);
			}
		}
		break;
	...
	case "comparesmall":
		if( parseFloat(compare1) > parseFloat(compare2) )
		{
			if( itemName.indexOf("^_^") != -1 ) {
				...
				// {0} 이(가) {1} 보다 큽니다.
				return this.ufnGetMessage("msg.err.validator.comparesmall",[columname1,columname2]);
			}
			// 비교대상 칼럼의 title을 넘겨주지 않았을 경우 해당 값을 표시
			else {
				//return titile + "의 값은 "+compare2+"보다 작아야 합니다.";
				// {0} 이(가) {1} 보다 큽니다.
				return this.ufnGetMessage("msg.err.validator.comparesmall",[titile,compare2]);
			}
		}
		break;
		...
		// 주민등록번호 체크 - isssn
	case "isssn":
		if (!this.lookupFunc("ufnIsSSN").call(itemValue))
		{
			//return this.ufnGetKoreanTarget(titile, "은(는)") + " 올바른 주민번호가 아닙니다.";
			// {0} 은(는) 올바른 주민번호가 아닙니다.
			return this.ufnGetMessage("msg.err.validator.ssn",[titile]);
			
		}
		break;
		// 외국인등록번호 체크 - isfrn
	case "isfrn":
		if (!this.lookupFunc("ufnIsFrnrIdNo").call(itemValue))
		{
			return this.ufnGetKoreanTarget(titile, "은(는)") + " 올바른 외국인등록번호가 아닙니다.";
		}
		break;
		// 사업자등록번호 체크 - isbzid
	case "isbzid":
		if (!this.lookupFunc("ufnIsBzIdNo").call(itemValue))
		{
			return this.ufnGetKoreanTarget(titile, "은(는)") + " 올바른 사업자등록번호가 아닙니다.";
		}
		break;
		// 법인등록번호 체크 - isfirmid
	case "isfirmid":
		if (!this.lookupFunc("ufnIsFirmIdNo").call(itemValue))
		{
			return this.ufnGetKoreanTarget(titile, "은(는)") + " 올바른 법인등록번호가 아닙니다.";
		}
		break;
		// 신용카드번호 체크 - iscardno
	case "iscardno":
		if (!this.lookupFunc("ufnIsCardNo").call(itemValue))
		{
			return this.ufnGetKoreanTarget(titile, "은(는)") + " 올바른 신용카드번호가 아닙니다.";
		}
		break;
		// 이메일 체크 - isemail
	case "isemail":
		if (!this.lookupFunc("ufnIsEmail").call(itemValue))
		{
			//return this.ufnGetKoreanTarget(titile, "은(는)") + " 올바른 이메일이 아닙니다.";
			// e-mail이 잘못된 형태로 입력 되었습니다.
			return this.ufnGetMessage("msg.err.validator.email",[titile]);
		}
		break;
...
* @return N/A
*/
pForm.ufnValidationCallback = function(sid, rtn)
{
	// Validation 오류시 focus 처리
	this.ufnSetValidationFocus(this.validationObject, this.validationColumn);
};
...
* @return N/A
*/
pForm.ufnSetValidationFocus = function(obj, bindcolumid)
{
	var binddataset;
	...
	// 데이터셋
		...
		if (!this.ufnIsNull(obj.bindgrid)) {
...
* @return N/A
*/
pForm.ufnResetValidationCss = function(validationarg)
{
    for(var m=0;m<validationarg.length;m++)
    {
        ...
        for(var j=0;j<this.binds.length;j++)
        {
            ...
            if(this.ufnIsNull(sBindDs)==true) continue;
...
*/
pForm.ufnGetKoreanTarget = function(itemName, option)
{
    if (option == "은(는)")
    {
...
* @return {Boolean}
*/
pForm.ufnIsDigit = function(sNum)
{
	var c;
	var point_cnt=0;
	var ret=true;
	
	if ( this.ufnIsNull(sNum) )	return false;
...
* @return {Boolean}
*/
pForm.ufnIsKoreanChar = function(strValue)
{
	var retVal = true;
...
* @return {Boolean}
*/
pForm.ufnIsSpecialChar = function(strValue)
{
	var retVal = false;
...
* @return {Boolean}
*/
pForm.ufnIsSSN = function(sJuminNo)
{
	var birthYear = this.ufnGetBirthYear(sJuminNo);
...
* @return {Boolean}
*/
pForm.ufnIsFrnrIdNo = function(strNo)
{
...
* @return {Boolean}
*/
pForm.ufnIsBzIdNo = function(strCustNo)
{
...
* @return {Boolean}
*/
pForm.ufnIsFirmIdNo = function(strNo)
{
	if (strNo.length != 13 || !isNumber(strNo)) return false;
...
* @return {Boolean}
*/
pForm.ufnIsCardNo = function(strNo)
{
...
* @return {Boolean}
*/
pForm.ufnIsEmail = function(strValue)
{
	var retVal = false;
	var sTmp = "";
...
/**
* @class 전화번호 형식에 맞는지 Check한다.
* @param {int} strValue
* @return {Boolean}
*/
pForm.ufnIsPhone = function(strNum)
{
	
	
	var num = strNum.match(/^010-?([0-9]{4})-?([0-9]{4})$/);
	if(num ==null){
		return false;
	}else{
		return true;   
...
/**
* @class 비밀번호 형식에 맞는지 Check한다.
* @param {int} strValue
* @return {Boolean}
*/
pForm.ufnIsPw = function(strPw)
{
	
	
	var pw = strPw.match(/^(?=.*[A-Za-z])(?=.*\d)(?=.*[@$!%*#?&])[A-Za-z\d@$!%*#?&]{8,}$/);
	if(pw ==null){
		return false;
	}else{
		return true;   
