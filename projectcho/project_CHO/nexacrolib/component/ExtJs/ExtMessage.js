 ...
 * @return N/A
 * @example
 * this.ufnAlert(this, "A", "확인하세요");	
 */
pForm.ufnAlert = function (sMsgId, arrArg, sPopId, sCallback)
{
	...
	if( this.ufnIsNull(sMsg)==true ) sMsg = "확인";
	// 줄바꿈 변경
	sMsg = sMsg.replace(/\\n/g, String.fromCharCode(10));
	sMsg =  pForm.ufnConvertMessage(sMsg, arrArg);
	
	var sMsgType = objApp.gdsMessage.lookup("MSG_ID", sMsgId, "MSG_TYPE");	
	if( this.ufnIsNull(sPopId)==true ) sPopId = sMsgId;
	...
	switch(sMsgType) {
		...
		case "C":
			...
			if(this.ufnIsNull(sCallback)) trace("callback함수를 지정하지 않았습니다");
			break;
	}
	...
	if (nexacro.getEnvironmentVariable("evMessagePopup") == "true") {
		this.ufnOpenPopup(sPopId,sMsgUrl,oArg,sCallback,oOption);
	}
	// alert-cofirm
	else {
		if (sMsgType == "A") {
 ...
 * @return {String}
 */
pForm.ufnConvertMessage = function(msg, values) 
{
 ...
 * @return {String}
 */
pForm.ufnGetMessage = function(sMsgId, arrArg) 
{
    var objApp = pForm.ufnGetApplication();
	if(objApp.gdsMessage.findRow("MSG_ID", sMsgId) < 0) return false;
