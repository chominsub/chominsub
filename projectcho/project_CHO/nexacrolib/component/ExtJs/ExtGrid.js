 ...
 * @return N/A
 * @example
 * this.ufnSetGrid(this.grdMain);	
*/
pForm.ufnSetGrid = function(objGrid)
{
	//Grid의 binddataset설정
	var objDs = objGrid.getBindDataset();
	
	// grid에 바인드된 Dataset이 없는 경우 return;
	if (this.ufnIsNull(objDs)) {
		return;
	}
	...
	if(this.ufnIsNull(arrProp)) return; 		//설정할 속성이 엄쪄용
	...
	this._ufnGridAddProp(objGrid);
	
	this._ufnMakeGridPopupMenu(objGrid,arrProp);//popupmenu 생성
	
	/*********************************************** 이벤트추가 START ***********************************************/	
	objGrid.addEventHandler("onheadclick", 	 this.ufnGrid_onheadclick, 	 this); 	//헤드클릭이벤트추가
	for( var k=0; k< arrProp.length; k++){
		...
		for( var n=0; n<arr.length; n++){
			if( arrProp[k] == arr[n]){
				...
				//우클릭이벤트추가
				objGrid.addEventHandler("onrbuttondown", this.ufnGrid_onrbuttondown, this);	    
				break;
			}
		}
		if( arrProp[k] == "cellcopypaste"){
			objGrid.addEventHandler("onkeydown", this.ufnGrid_onkeydown, this);
		}
	}
	/*********************************************** 이벤트추가 END *************************************************/
 ...
 * @return N/A
 * @example
 * this._ufnGridAddProp(this.grdMain);	
*/
pForm._ufnGridAddProp = function (objGrid)
{
	...
	for( var i=0; i<arrProp.length; i++)
	{
		switch(arrProp[i]) {
			case "checkbox":
				this._ufnGridCheckboxNoStatusAdd(objGrid, objDs, "checkbox");
				break;
			case "no":
				this._ufnGridCheckboxNoStatusAdd(objGrid, objDs, "no");
				break;
			case "status":
				this._ufnGridCheckboxNoStatusAdd(objGrid, objDs, "status");
				break;
			case "sort":
				objGrid.sort = "true";
				break;
 ...
 * @return N/A
 * @example
 * this._ufnGridCheckboxNoStatusAdd(this.grdMain, this.dsList, [checkbox,no,status]);	
*/
pForm._ufnGridCheckboxNoStatusAdd = function (objGrid, objDs, addProp)
{	
	var nHeadColIndex;
	if(this.ufnIsNull(objDs.insertheadcell)) nHeadColIndex = 0;
	else nHeadColIndex = objDs.insertheadcell;	

	var nBodyColIndex;
	if(this.ufnIsNull(objDs.insertbodycell)) nBodyColIndex = 0;
	...
	var sNo = this.ufnGetWord("cmm.no");		// 순번
	var sStatus = this.ufnGetWord("cmm.status");// 상태
	...
	if( addProp == "checkbox")
	{
		...
		for( var i=0; i<objGrid.getCellCount("head"); i++){
			...
			if( tmp == "0"){
				...
				var bodyCellIndex = this._ufnGridGetBodyCellIndex(objGrid, i);
				// body cell index 에 해당하는 바인드 컬럼명
				var columnName = this._ufnGridGetBindColumnNameByIndex(objGrid, bodyCellIndex);
	...
	if ( addProp == "status")
	{
		...
		var sInsert = nexacro.wrapQuote(this.ufnGetWord("insert")); //입력
		var sUpdate = nexacro.wrapQuote(this.ufnGetWord("modify")); //수정
		var sDelete = nexacro.wrapQuote(this.ufnGetWord("delete")); //삭제
		var sExpr = "expr:"
 ...
 * @return  N/A
 * @example
 * objGrid.addEventHandler("onheadclick", 	 this.ufnGrid_onheadclick, 	 this);
 */
pForm.ufnGrid_onheadclick = function(objGrid, e)
{
	...
	if (sType == "checkboxcontrol"){
		...
		this._ufnHeadCheckSelectAll(objGrid, e);
	}else{
		//sort
		if(this.ufnIsNull(objGrid.sort) || objGrid.sort=="false"){
			return;
		}else if(objGrid.sort == "true"){
			var arr = objGrid.arrprop;
			var bUserHeader = this._ufnGridUserHeaderFlg(objGrid);
			var multiple = false;
			if ( e.ctrlkey ) multiple = true;// Ctrl 키
			if(!bUserHeader){
				// 정렬 상태 변경이 성공하면 정렬을 실행한다.
				var rtn = this._ufnGridSetSortStatus(objGrid, e.cell, multiple);
				if(rtn){
					this._ufnGridExecuteSort(objGrid);
				}
			}else{
				this._ufnGirdUserHeaderExcuteSort(objGrid, e.cell, multiple);
			}
		}
	}
};
 ...
 * objGrid.addEventHandler("onheadclick", 	 this.ufnGrid_onheadclick, 	 this);
 */
pForm.ufnGrid_onkeydown =function(objGrid, e){
	...
	if(e.ctrlkey){
		if(keycode == 67){
			...
			if( sBrowser == "nexacro" || sBrowser == "IE"){
				this._ufnGridCopyEventForRuntime(objGrid, e);
			}else {
				this._ufnGridCopyEventForChrome(objGrid, e);
			}
		}else if(keycode == 86){
			//paste
			this._ufnGridPasteEvent(objGrid, e);
		}
	}
};
 ...
 * @example
 * this._ufnGridUserHeaderFlg(this.grdMain);
 */
pForm._ufnGridUserHeaderFlg = function (objGrid)
{
	var arr = objGrid.arrprop;
 ...
 * @return N/A
 * @example
 * this._ufnGirdUserHeaderExcuteSort(objGrid);
 */
pForm._ufnGirdUserHeaderExcuteSort = function (objGrid, headCellIndex, multiple)
{
	var bindCol = objGrid.getCellProperty("head", headCellIndex, "calendarweekformat");
	if( this.ufnIsNull(bindCol)) return false; //헤더에 바인드없음
	...
	var rtn = this._ufnGridSetSortStatus(objGrid, headCellIndex, multiple, "", bodyCellIdx);
	if(rtn){
		this._ufnGridExecuteSort(objGrid);
	}
};
 ...
 * @example
 * this._ufnGridSetSortStatus(obj, e.cell, multiple);	
 */
pForm._ufnGridSetSortStatus = function(grid, headCellIndex, isMultiple, sortStatus, bodyCellIndex)
{
	// head cell index 에 해당하는 body cell index
	if( this.ufnIsNull(bodyCellIndex)){
		bodyCellIndex = this._ufnGridGetBodyCellIndex(grid, headCellIndex);
	}
	if ( bodyCellIndex < 0 ) return false;
	
	// body cell index 에 해당하는 바인드 컬럼명
	var columnName = this._ufnGridGetBindColumnNameByIndex(grid, bodyCellIndex);
	if ( this.ufnIsNull(columnName) ){
		trace("Check Grid body cell bind value");
		return false;
	}
	
	if ( this.ufnIsNull(isMultiple) ) isMultiple = false;
	if ( this.ufnIsNull(sortStatus) ) sortStatus = -1;
	
	// 대상 grid 에 정렬정보를 가지는 사용자 속성 확인/추가
	if ( this.ufnIsNull(grid.sortInfos) ){
		grid.sortInfos = {};
	}
	
	// 정렬대상컬럼 (순서중요)
	if ( this.ufnIsNull(grid.sortItems) ){
		grid.sortItems = [];
	}
	...
	if ( this.ufnIsNull(sortInfo) )
	{
		...
		var refCell = this._ufnGridGetGridCellObject(grid, "head", headCellIndex);
 ...
 * this._ufnGridSetSortStatus(obj, e.cell, multiple);	
 */ 
pForm._ufnGridGetBodyCellIndex = function(grid, headCellIndex, useColspan) 
{	//, useColspan) 
	if( this.ufnIsNull(useColspan)) useColspan=false;
	// Max Head Row Index
	var maxHeadRow = 0;
 ...
 * @example
 * this._ufnGridGetBindColumnNameByIndex(obj, e.cell);	
 */  
pForm._ufnGridGetBindColumnNameByIndex = function(grid, index) 
{
	var text = "";
	var columnid = "";
 ...
 * @return{String}  N/A
 * @example
 * this._ufnGridExecuteSort(obj);	
 */  
pForm._ufnGridExecuteSort = function(grid) 
{
	var sortInfo, 
		sortItem,
	...
	if ( this.ufnIsNull(sortInfos) || this.ufnIsNull(sortItems) ) return;
	...
	for (var i=0; i<sortItems.length; i++) {
		...
		// 확인하여 없으면 제거해 준다.
		if ( this.ufnIsNull(cell) || grid.getBindCellIndex("body", columnName) < 0 ){
			// 컬럼정보제거
	...
	for (var p in sortInfos) {
		if ( sortInfos.hasOwnProperty(p) )
		{
			...
			{
				index = cell._cellidx;
				marker = this.ufnDecode(sortInfo.status, 1, this.MARKER[0], 2, this.MARKER[1], "");
 ...
 */
pForm._ufnGridGetGridCellObject = function(grid, band, index)
{
	// 내부속성을 통해 얻어온다.
	var refCell;
 ...
 * @return N/A
 */  
pForm._ufnClearSortMark = function(obj)
{
 ...
 * @return  N/A
 * @example
 * this._ufnGetHeadBodyIndex(this.grdMain, this.dsMain);	
 */
pForm.ufnGrid_onrbuttondown = function (objGrid, e)
{
	var objApp = pForm.ufnGetApplication();
...
/**
 * @class  ufnCreatePopupMenu 내부함수로 팝업메뉴 클릭 시 발생하는 이벤트
 * @param {Object} objGrid	- 대상그리드
 * @param {Evnet}  e 		- 팝업메뉴클릭이벤트
 * @return N/A
 * @example
 * this.ufnPopupmenu_onmenuclick(this.grdMain, nexacro.MenuClickEventInfo);	
 */
pForm.ufnPopupmenu_onmenuclick = function (objMenu, e)
{
	var selectId   = e.id;
	...
	switch(selectId) {
		case "colfix"://틀고정 열
			...
			this._ufnGridcellFix(grid, this.fv_CellIndex, nRowIndex);
			break;
		case "colfixfree"://틀고정 열 해제
			this._ufnGridCellFree(grid);
			break;
		case "rowfix"://틀고정 행
			if(nRowIndex<0) return;
			grid.fixedRow = nRowIndex;
			this._ufnGridSetCssclass(grid);
			break;
		case "rowfixfree"://틀고정 행 해제
			...
			break;
		case "filter"://필터
			this._ufnGridFilter(grid);
			break;
		case "filterfree"://필터해제
			this._ufnGridCellFilterFree(grid);
			break;
		case "replace"://찾기/바꾸기
			this._ufnGridCellReplace(grid, nCellIndex, nRowIndex);
			break;
		case "colhide"://컬럼숨기기
			this._ufnGridColHideShow(grid, nRowIndex);
			break;	
		case "export"://엑셀내보내기
			this._ufnGridExcelExport(grid, nRowIndex);
			break;	
		case "import"://엑셀가져오기
			this._ufnGridExcelImport(grid, nRowIndex);
			break;	
		case "personal" : //개인화
			this._ufnGridPersonalize(grid);
			break;
		case "initial"://초기화
			grid.set_formats("<Formats>" + grid.orgformat2 + "</Formats>");
			//this._ufnGridCellFree(grid);
			//this._ufnClearSortMark(grid);
			this._ufnGridAddProp(grid);
			break;
		default: break;
	}
};
...
/**
 * @class  _ufnGridSetCssclass 행고정/해제시 css설정
 * @param {Object} objGrid	- 대상그리드
 * @return N/A
 * @example
 * this._ufnGridSetCssclass(this.grdMain);	
 */
pForm._ufnGridSetCssclass = function (objGrid)
{
 ...
 * @return  N/A
 * @example
 * this._ufnHeadCheckSelectAll(objGrid, e); //ALL CHECK
 */
pForm._ufnHeadCheckSelectAll = function (objGrid, e)
{
	...
	if (sVal == "1" ){
		...
		var bodyCellIndex = this._ufnGridGetBodyCellIndex(objGrid, nHeadCell);
		// body cell index 에 해당하는 바인드 컬럼명
		var columnName = this._ufnGridGetBindColumnNameByIndex(objGrid, bodyCellIndex);
		if(columnName == "gridcmmcheck") {
			 sChk="";
 ...
 * this._ufnGetHeadBodyIndex(this.grdMain, this.dsMain);	
 */
pForm._ufnMakeGridPopupMenu = function (objGrid, arrProp)
{
	var objApp 		 = pForm.ufnGetApplication();
	...
	for (var i=0; i<arrProp.length; i++) {
		for (var j=0; j<objPopupDs.rowcount; j++){
			...
			if (this.ufnIsNull(sMenu)) continue;
	...
	objPopMenu.set_levelcolumn("level");
 	objPopMenu.addEventHandler("onmenuclick", this.ufnPopupmenu_onmenuclick, objParentForm);
	objPopMenu.show();
...
pForm._getGridUserProperty = function (objGrid)
{
	...
	for( var i=0; i< arrdefault.length; i++){
		if(!this.ufnIsNull(arrdefault[i])){
			arrmyprop.push(arrdefault[i]);
		}
	}
	
	for( var i=0; i< arrprop.length; i++){
		if(!this.ufnIsNull(arrprop[i])){
 ...
 * @return N/A
 * @example
 * this._ufnGridcellFix(this.grdMain, 1, 2);	
 */
pForm._ufnGridcellFix = function (objGrid, nCellIdx, nRowIdx)
{
	var sBandType;
 ...
 * @return N/A
 * @example
 * this._ufnGridCellFree(this.grdMain);	
 */
pForm._ufnGridCellFree = function(objGrid)
{
 ...
 * @return N/A
 * @example
 * this._ufnGridFilter(this.grdMain);	
 */
pForm._ufnGridFilter = function(objGrid)
{
	var sTitle = this.ufnGetWord("popup.datafiltersetting");
	var oArg = {pvGrid:objGrid};
	
	var oOption = {title:sTitle};	//top, left를 지정하지 않으면 가운데정렬 //"top=20,left=370"
	var sPopupCallBack = "ufnGridFilterCallback";
	this.ufnOpenPopup( "cmmGridFilter", "Cmm::CmmGridFilter.xfdl",oArg, sPopupCallBack, oOption);	
};
 ...
 * @return N/A
 * @example
 * this._ufnGridCellFilterFree(this.grdMain);	
 */
pForm._ufnGridCellFilterFree = function(objGrid)
{
	var objDs = objGrid.getBindDataset();
	objDs.set_filterstr("");
};
 ...
 * @return N/A
 * @example
 * this._ufnGridCellReplace(this.grdMain);	
 */
pForm._ufnGridCellReplace = function(objGrid,nCellIndex,nRowIndex)
{
	var sTitle = this.ufnGetWord("popup.datafindreplace");
	...
	var oOption = {title:sTitle};	//top, left를 지정하지 않으면 가운데정렬 //"top=20,left=370"
	var sPopupCallBack = "ufnReplaceCallback";
	this.ufnOpenPopup( "cmmFindReplace","Cmm::CmmFindReplace.xfdl",oArg,sPopupCallBack,oOption);	
};
 ...
 * @return N/A
 * @example
 * this._ufnGridColHideShow(this.grdMain);	
 */
pForm._ufnGridColHideShow = function(objGrid)
{
	var sTitle = this.ufnGetWord("popup.colshwohide");
	
	var oArg = {pvGrid:objGrid};
	var oOption = {title:sTitle};	//top, left를 지정하지 않으면 가운데정렬 //"top=20,left=370"
	var sPopupCallBack = "ufnColumnHidCallback";
	this.ufnOpenPopup( "cmmColumnHide","Cmm::CmmColumnHide.xfdl",oArg,sPopupCallBack,oOption);	
};
 ...
 * @return N/A
 * @example
 * this._ufnGridExcelExport(this.grdMain);	
 */
pForm._ufnGridExcelExport = function(objGrid)
{
	this.ufnExcelExport(objGrid, "*?*?*?*?*?*?*?","");
};
 ...
 * @return N/A
 * @example
 * this._ufnGridExcelImport(this.grdMain);	
 */
pForm._ufnGridExcelImport = function(objGrid)
{
	var sDataset = objGrid.binddataset;
	this.ufnExcelImport(sDataset, "sheet1", "A2", "fnImportCallback", objGrid.name + sDataset , this);
};
 ...
 * @return N/A
 * @example
 * this._ufnGridPersonalize(this.grdMain);	
 */
pForm._ufnGridPersonalize = function(objGrid)
{
	var sOrgFormat = objGrid.orgformat2;
	var sCurFormat = objGrid.getCurFormatString();
	this._ufnGridPersonalizeExcute(objGrid);
//	//변경된 사항 확인 할 경우 아래 스크립트 사용
// 	if( sOrgFormat == sCurFormat ){
// 		this.ufnAlert("msg.save.nochange","","NoChangeFormat");
// 	}else{
// 		var sId = "ChangeFormat|" + objGrid.name;
// 		this.ufnAlert("confirm.before.save","", sId, "ufnGridFormatChangeMsgCallback");
// 	}
};
 ...
 * this._ufnGridPersonalize(this.grdMain);	
 */
pForm._ufnGridPersonalizeExcute = function (objGrid)
{
	var sFormatId 	= this._getUniqueId(objGrid);
	...
	this.ufnAlert("msg.save.success","","saveSuccess","ufnGridFormatChangeMsgCallback");
};
//////////////////////////////////////////////////////////////////////////POPUPMENU CALLBACK///////////////////////////////////////////////////////////
 ...
 * @return N/A
 * @example
 * this.ufnGridFormatChangeFormatCallback("TEST", "");	
 */
pForm.ufnGridFormatChangeMsgCallback = function (sid, rtn)
{
	//TODO.
};
 ...
 * @return N/A
 * @example
 * this.ufnReplaceCallback("TEST", "");	
 */
pForm.ufnReplaceCallback = function (sid, rtn)
{
	//TODO
};
 ...
 * @return N/A
 * @example
 * this.ufnGridFilterCallback("TEST", "");	
 */
pForm.ufnGridFilterCallback = function (sid, rtn)
{
	//TODO
};
 ...
 * @return N/A
 * @example
 * this.ufnColumnHidCallback("TEST", "");	
 */
pForm.ufnColumnHidCallback = function (sid, rtn)
{
	//TODO
};
 ...
 * @example
 * this.ufnFindGridText(this.fv_grid, txt, option);
 */
pForm.ufnFindGridText = function (grid, findText, option)
{
	...
	if ( scope == "col" && !this.ufnIsNull(option.cell) )
	{
 ...
 * @example
 *this.ufnReplaceGridText(grid, findText, replaceText, option, bAllChg);
 */
pForm.ufnReplaceGridText = function(grid, findText, replaceText, option, all)
{
	// F3 발생 시 마지막 찾은 문자열 계속 찾기 위해 값 지정
	grid.lastFindText = findText;
	grid.lastFindOption = option;
	
	if ( this.ufnIsNull(all) )
	{
		all = false;
	}
	...
	var dataset = grid.getBindDataset();//this.ufnLookup(grid.parent, grid.binddataset);
	...
	else
	{
		...
		if ( this.ufnIsNull(lastReplaceRow) )
		{
			startRow = 0;
		}
		else
...
pForm._replaceGridCellText = function(grid, findRow, findCell, findText, replaceText, strict)
{
	...
	var dataset = grid.getBindDataset();//this.ufn_Lookup(grid.parent, grid.binddataset);
...
// 	}
	...
	var dataType = this.ufnGetBindColumnType(grid, findCell);
	if ( this.ufnIsNull(displayType) || displayType == "normal" )
	{
		switch(dataType)
		{
			case 'INT' :
	...
	if ( displayType == "number" || displayType == "currency" )
	{
		...
		{
			displayValue = this.ufnReplaceIgnoreCase(displayValue, findText, replaceText);
		}		
	...
	else
	{
		...
		{
			replaceVal = this.ufnReplaceIgnoreCase(columnValue, findText, replaceText);
		}					
	}
 ...
 * @example
 * this.ufnReplaceIgnoreCase(str, findStr, "x");
 */
pForm.ufnReplaceIgnoreCase = function( sOrg, sRepFrom, sRepTo )	
{
...
{		
	...
	{
		replaceDisplayValue = this.ufnReplaceIgnoreCase(displayValue, findText, replaceText);
	}
	
	// 바꿀 값이 없다면 값을 제거한다.
	if ( this.ufnIsNull(replaceDisplayValue.trim()) )
	{
		return [true, null];
	}
	...
	// 일자 유효 체크
	if ( !this.ufnIsNull(checkYearValue) && 
	     !this.ufnIsNull(checkMonthValue) && checkDayIndex > -1 )
	{
		var dt = checkYearValue + checkMonthValue + "01";
		var inputDay = parseInt(dateValue[checkDayIndex]);
		var lastDay = this.ufnGetMonthLastDay(dt);
	}
...
pForm._compareFindText = function(grid, row, cell, findText, condition, strict)
{
	...
	if( this.ufnIsNull(cellText))return;
	...
	if ( this.ufnIsNull(displayType) || displayType == "normal" )
	{
		var dataType = this.ufnGetBindColumnType(grid, cell);
		switch(dataType)
		{
			case 'INT' :
			case 'FLOAT' :
 ...
 * @return {Object} - 찾기 성공.
 * @example
 *  this.ufnGetBindColumnType(grid, cell);
 */
pForm.ufnGetBindColumnType = function(grid, cell)
{
	var dataType = null;
	var dataset = this.ufnLookup(grid.parent, grid.binddataset);
	var bindColid = grid.getCellProperty("body", cell, "text");
		bindColid = bindColid.replace("bind:", "");
	
	if ( !this.ufnIsNull(bindColid) )
	{
		var colInfo = dataset.getColumnInfo(bindColid);
		if ( !this.ufnIsNull(colInfo) )
		{
 ...
 * @return N/A
 * @example
 * this._ufnGridCopyEventForRuntime(obj, e);	
*/
pForm._ufnGridCopyEventForRuntime = function (obj, e)
{
 ...
 * @return paste데이터 
 * @example
 * this._ufnGirdGetPasteData("nexacro");	
*/
pForm._ufnGirdGetPasteData = function (browser)
{
	var copyData = "";
 ...
 * @return N/A
 * @example
 * this._ufnGridPasteEvent(obj, e);	
*/
pForm._ufnGridPasteEvent = function (obj, e)
{
	var browser = system.navigatorname;
	var copyData = this._ufnGirdGetPasteData(browser);
		
	if( this.ufnIsNull(copyData)){
		var copyData ="";
 ...
 * @return N/A
 * @example
 * this._ufnGridCopyEventForChrome(obj, e);	
*/
pForm._ufnGridCopyEventForChrome = function (obj, e)
{
	var startrow = nexacro.toNumber(obj.selectstartrow);
	if( startrow == -9) return;
 ...
 * @return 	 N/A
 */
pForm.ufnsetGridAppend = function(grid, dataset, targetfunction, pagesize, objCnt, btnNext)
{
    ...
    if (btnNext)
    {
		...
		// 다음 버튼시 조회 이벤트 생성
		btnNext.addEventHandler("onclick", this.ufnGridAppendNextBtn, this);
	}
    ...
    grid.addEventHandler("onvscroll", this.ufnGridAppend, this);
};
 ...
 * @return 	 N/A
 */
pForm.ufnGridAppend = function(obj, e)
{
    if (e.pos == obj.vscrollbar.max)
    {		
        // 스크롤 이벤트의 중복을 막기 위해서 이벤트 제거
        obj.removeEventHandler("onvscroll", this.ufnGridAppend, this);
 ...
 * @return 	 N/A
 */
pForm.ufnGridAppendNextBtn = function(obj,  e)
{	
    var objGrd = obj.appendGrid;
	
    // 스크롤 이벤트의 중복을 막기 위해서 이벤트 제거
    objGrd.removeEventHandler("onvscroll", this.ufnGridAppend, this);
 ...
 * @return 	 N/A
 */
pForm.ufnAppendGridData = function(targetgrid, appendDataset)
{
	...
	if (this.ufnIsNull(targetdataset.keystring) == false) {
    ...
    if (!noMore) {	
        // 그리드의 onvscroll 이벤트 추가
        targetgrid.addEventHandler("onvscroll", this.ufnGridAppend, this);
    }
    // 더 이상 다음 레코드가 없을 경우
    else {
        // 그리드의 onvscroll 이벤트 삭제
        targetgrid.removeEventHandler("onvscroll", this.ufnGridAppend, this);
    }	
    // append dataset 초기화
 ...
 * @return 	 N/A
 */
pForm.ufnResetGridAppend = function(targetgrid, dataset)
{
	// Grid dataset 초기화
