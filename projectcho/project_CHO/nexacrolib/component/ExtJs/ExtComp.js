 ...
 */
pForm.ufnGetCompId = function (obj)
{
 ...
 * @return N/A
 */
pForm.ufnCalMMOndropdown = function (obj)
{
	var pdvName = this.ufnGetCompId(obj);
	
	// Creating pdv
	if (this.ufnIsNull(this.components[pdvName])) {
 ...
 * @return N/A
 */
pForm.ufnSetAccordion = function(objList, openMode)
{
    ...
    for(var objName in objList)
    {
        ...
        // 대상아코디언이 있으면
        if( !this.ufnIsNull(objList[objName]) )
        {
            // Div Object일경우
    ...
    for(var i=0;i<targetlist.length;i++)
    {
        ...
        if( this.ufnIsNull(targetlist[i].accordionbody) )
        {
        ...
        for(var j=0;j<headobjects.length;j++)
        {
            ...
            {
                // 아코디언 이벤트 처리핸들
                headobjects[j].addEventHandler("onclick", this._ufnHandleAccordionButtonClick, this);
                headobjects[j].accordionlist = targetlist;
                headobjects[j].accordionhead = targetlist[i];
                //headobjects[j].setFocus(false);

                if( this.ufnIsNull(firstbtnobj) )
                {
                    firstbtnobj = headobjects[j];
                }

                if( this.ufnIsBoolean(openMode))
                {
    ...
    // 현재의 상태에 맞게 리드로잉처리한다.
    this._ufnRedrawAccordion(firstbtnobj);
};
 ...
 * @return N/A
 */
pForm._ufnHandleAccordionButtonClick = function(obj,e)
{
 ...
 * @return N/A
 */
pForm._ufnRedrawAccordion = function(obj)
{
	//trace("===== _ufnRedrawAccordion : " + obj.name);
    var heightoffset  = 0;
    ...
    {
		//trace("===== _ufnRedrawAccordion accordionlist[i].name : " + accordionlist[i].name);
        ...
        {
            // 바디가 존재할때
            if( !this.ufnIsNull(accordionlist[i].accordionbody) )
            {
 ...
 * @return N/A
 */
pForm._ufnSetEditMsClear = function (oEdt)
{
 ...
 * @return N/A
 */
pForm._ufnSetEditMsClearRemove = function (oEdt)
{
	var _oParent = oEdt.parent;
 ...
 * @return N/A
 */
pForm.ufnSetEditMsClear = function (oEdt, bMsClear)
{	
	if (this.ufnIsNull(oEdt._ms_clear) || !oEdt._ms_clear) 
		oEdt._ms_clear = true;
	
	if (!this.ufnIsNull(bMsClear)) 
		oEdt._ms_clear = bMsClear;	
	
	  if (oEdt._ms_clear)
		pForm._ufnSetEditMsClear(oEdt);	
	else
		pForm._ufnSetEditMsClearRemove(oEdt); // 해당 컴포넌트 X버튼 삭제	
};
 ...
 * @return N/A
 */
pForm.ufnSetEditMsClearAll = function (oForm, bMsClear)
{		
	...
	for (var i=0; i<nLength; i++)
	{
		if (arrComp[i] instanceof nexacro.Div) {
			this.ufnSetEditMsClearAll(arrComp[i].form, bMsClear); //재귀함수
		}
			...
			if (arrComp[i] instanceof nexacro.Edit) {
				...
				if (!this.ufnIsNull(bMsClear)) oEdt._ms_clear = bMsClear;	
 ...
 */
pForm.ufnGetDLLPath = function(sFileName)
{
	var strpath = "";
...
trace(strpath);	
	//trace("==================== ufnGetDLLPath strpath : " + strpath);
    return strpath;
};
 ...
 */ 
pForm.ufnOpenDLL = function(sFileName)
{
	var objApp = this.ufnGetApplication();
	
	if (this.ufnIsNull(objApp[sFileName])) {
		objApp[sFileName] = {};
	}
	else {
	...
	var strpath = this.ufnGetDLLPath(sFileName);
 ...
 * @return N/A
 */  
pForm.ufnCloseDLL = function(sFileName)
{
	// 로컬/웹 접근에 따른 파일 full 경로 가져오기
	var strpath = this.ufnGetDLLPath(sFileName);
 ...
 */ 
pForm.ufnGetExtCommon = function()
{
	var strFile = "";
...
// 	}
	...
	trace("==================== ufnGetExtCommon strFile : " + strFile);
	return this.ufnOpenDLL(strFile);
 ...
 * @return N/A
 */  
pForm.ufnCloseExtCommon = function()
{
	var strFile = "";
