 ...
 * @return N/A
 * @example
 * this.ufnOpenPopup(this);
 */
pForm.ufnOpenPopup = function (sPopupId, sUrl, oArg, sPopupCallback, oOption)
{
	...
	var sPopupCallback = (this.ufnIsNull(sPopupCallback) && this["fnPopupCallback"]) ? "fnPopupCallback" : sPopupCallback;
	...
	if (this.ufnIsNull(oArg) == true) oArg = {};
    ...
    if(sPopupType == "modeless")
    {
		...
		if(!this.ufnIsNull(sTitleText)) newChild.set_titletext(sTitleText);
		...
		{			
			// modalsync 팝업은 return이 없어 ufnClosePopup 함수에서 셋팅한 값으로 리턴처리
...
*/
pForm.ufnClosePopup = function(objRtn)
{
