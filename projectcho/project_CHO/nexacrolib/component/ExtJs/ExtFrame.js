...
// 			
// 			// callBack 함수가 있을 때
// 			if (this.ufnIsNull(sCallBack) == false) {			
// 				// onclose 이벤트 추가
		...
		if (arrComp[i] instanceof nexacro.Div){
			...
			if (arrComp[i] instanceof nexacro.Edit)	{
				...
				if (arrComp[i]._ms_clear == "true") {
					this._ufnSetEditMsClear(arrComp[i]);
				}
			}
			...
			if (arrComp[i] instanceof nexacro.Calendar) {
				...
				if (arrComp[i].uCalType == "MM") {
					arrComp[i].addEventHandler("ondropdown", this.ufnCalMMOndropdown, this);
				}
			}
		}
	}
 ...
 * @example 
 */
pForm.ufnCall = function(oObj)
{	
	if(!this.ufnIsNull(oObj) && typeof(oObj) !=  "object") return;	
	...
	if (!this.ufnIsNull( oObj.sMenuId)){
	...
	this.ufnNewMdi(sMenuId, nRow, oArgs);
};
 ...
 * @example 
 */
pForm.ufnCallSDI = function(oObj)
{	
	if (!this.ufnIsNull(oObj) && typeof(oObj) !=  "object") return;	
 ...
 * @return N/A
 */
pForm.ufnNewMdi = function(sMenuId, nRow, oArgs)
{	
	var objApp   = pForm.ufnGetApplication();
	var gdsOpen  = objApp.gdsOpenMenu;		//열린 dataset
	...
	if(this.ufnIsNull(sPageUrl)) return;		//pageURl 이 없으면 return
	this.ufnSetOpenMenuDs(winid, sMenuId, sMenuNm, sPageUrl, sGroupId);	// 열린메뉴 화면 삽입

	var objNewWin = new ChildFrame;
	objNewWin.init(winid, 0, 0, objApp.gvWorkFrame.getOffsetWidth(), objApp.gvWorkFrame.getOffsetHeight());
 ...
 * @return N/A
 */
pForm.ufnNewSdi = function(sMenuId, oArgs)
{
	var objApp   = pForm.ufnGetApplication();
	var gdsOpen  = objApp.gdsOpenMenu;		//열린 dataset
	var gdsMenu  = objApp.gdsMenu;
	...
	if(this.ufnIsNull(sPageUrl)) return;
 ...
 * @return N/A
 */
pForm.ufnSetOpenMenuDs = function(winid, menuid, strTitle, spageUrl, sGroupId)
{
	var objApp  = pForm.ufnGetApplication();
	var gdsOpen = objApp.gdsOpenMenu ;  //열린 dataset
