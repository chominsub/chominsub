 ...
 * @return N/A
 * @example 
 * this.ufnFileUpload(objFileUpload);
 */
pForm.ufnFileUpload = function(objFileUpload, sUrl, sPath)
{	
	var svcUrl = this.ufnGetServerUrl();
	
	if (this.ufnIsNull(sUrl)) sUrl = svcUrl;
	
	//파일업로드 서비스 호출 경로
	var sFileUrl = sUrl + "fileUpload.jsp";
    
	//파일 업로드 시킬 폴더 위치 지정
	if (this.ufnIsNull(sPath)) sPath = "PATH=upload";
 ...
 * @param {String} [sPath] - 파일업로드시킬 폴더 위치
 * @return N/A
 * @example this.ufnFileUpload(objFileUpload, sFilename);
 */
pForm.ufnFileDownload = function(objFileDownload, sFilename, sUrl, sPath)
{
	var svcUrl = this.ufnGetServerUrl();
	if (this.ufnIsNull(sUrl)) sUrl = svcUrl;
 ...
 * 실패 : "" <br>
 */
pForm.ufnGetFileName = function (sPath, bExt)
{
	var start_pos,end_pos,tmp_pos,filename;

	if (this.ufnIsNull(sPath)) 
	{
		return "";
	}
	if (this.ufnIsNull(bExt)) 
	{
		bExt = false;
	}

	start_pos = Math.max(this.ufnPosReverse(sPath, "\\"), this.ufnPosReverse(sPath, "/"));
	tmp_pos = this.ufnPosReverse(sPath, "::");
	if (tmp_pos > 0) 
	{
		tmp_pos++;
	}
	...
	if (bExt == false) 
	{
		end_pos = this.ufnPosReverse(sPath, ".");
		if (end_pos < 0) 
		{
...
*/
pForm.ufnInitExtToIcon = function ()
{
...
*/
pForm.ufnGetFileIcon = function(fileName)
{
	if(this.ufnIsNull(fileName)) return;
...
*/
pForm.ufnSizeToByte = function(fileSize) 
{
