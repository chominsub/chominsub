 ...
 * @example
 * var bNull = this.ufnIsNull("aaa");	// false
 */
 ...
 */
pForm.ufnNvl = function(inVal, nullVal)
{
	if(inVal == null) {
		inVal = nullVal;
	}
...
*/
pForm.ufnIsBoolean = function(obj)
{
    var strvalue = obj+"";
 ...
 * this.ufnDecode();	
 */
pForm.ufnDecode = function ()
{
	var varRtnValue = null;

	var arrArgument = this.ufnDecode.arguments;
 ...
 * @return {String} 문자열
 */
pForm.ufnTrim = function(sValue)
{
    if (this.ufnIsNull(sValue)) return "";
 ...
 * @return {String} 문자열
*/
pForm.ufnAllTrim = function(sValue)
{
 ...
 * @return {String} 문자열
 */
pForm.ufnLeft = function (sOrg, nSize)
{
 ...
 * @return {String} 결과값
 */
pForm.ufnRight = function(sOrg, nSize)
{
	if ( this.ufnIsNull(sOrg) || this.ufnIsNull(nSize) )	return "";
 ...
 */
pForm.ufnLengthByte = function (sVal)
{
	var lengthByte = 0;

	if (this.ufnIsNull(sVal)) {
		return -1;
	}
 ...
 * @return {Number}
 * @example
 * var nPos = this.ufnPosReverse("aaBBbbccBB", "BB"); <br>
 ...
 */
pForm.ufnAppendComma = function(nNumber,nDetail)
{
 ...
 */
pForm.ufnRemoveComma = function(strValue)
{
 ...
 * @return {String} 숫자문자열
 */
pForm.ufnGetDigit = function(strValue)
{
 ...
 */
pForm.ufnRemoveSpecialChar = function(strValue)
{
   var strSpecial = "~!@#$%^&*-+./=_`{|}()\\?<>";
 ...
 * @return {String}
 * @example
 * var str = this.ufnRemoveHtmlTag("정상적으로<BR>처리되었습니다.");
 */
pForm.ufnRemoveHtmlTag = function(sHtml, sTag)
{
	if(this.ufnIsNull(sTag))
	{
 ...
 */
pForm.ufnIsExistInArray = function(arrVal, varVal) 
{
	var retVal = false;
 ...
 */
pForm.ufn16To10 = function(strVal)
{
	var n;
...
* @example
* trace(this.ufnTypeOf(Button00));	// output : Button
* trace(this.ufnTypeOf(Tab00));	// output : Tab
* trace(this.ufnTypeOf(Tab00.tabpage1));	// output : Tabpage
* trace(this.ufnTypeOf(Dataset00));	// output : Dataset
*/
pForm.ufnTypeOf = function(obj)
{
	var type;
 ...
 * @return {boolean}
 */
pForm.ufnIsNexaComponent = function(value) 
{
 ...
 * @return N/A
 */
pForm.ufnObjView = function (objArg, sName)
{
	for (var name in objArg)
	{
		if (this.ufnIsNull(sName) == false)
		{
			if (name == sName) {
 ...
 * @return  {Object} 오브젝트
 * @example
 * this.ufnLookup(this, "dsList");	
 */
pForm.ufnLookup = function(p, name)
{
	var o;
	while (p)
	{		
 ...
 * var index = this.ufnLastIndexOfProp(users, "name", "lee");
 * trace("index==>" + index);	// output : index==>6
 * var index = this.ufnLastIndexOfProp(users, "name", "lee", 5);
 * trace("index==>" + index);	// output : index==>3
*/
pForm.ufnLastIndexOfProp = function(array, prop, item, from, strict)
{
	var i, obj, 
		propValue;
...
*/
pForm.ufnRemoveAt = function(array, index) 
{
 ...
 * @return {String} id
 * @example
 * trace(this.ufnGetUniqueI()); 
 * // output : 3e52d1f6-f0d2-4970-a590-ba7656b07859
 * 
 * trace(this.ufnGetUniqueI("Button_")); 
 * // output : Button_4e601da1-63f4-4cfa-849b-01b8a7f14d40
 * 
 * trace(this.ufnGetUniqueI("", "_")); 
 * // output : 4e601da1_63f4_4cfa_849b_01b8a7f14d40
 * 
 * trace(this.ufnGetUniqueId("Button_", "_")); 
 * // output : Button_4e601da1_63f4_4cfa_849b_01b8a7f14d40
*/
pForm.ufnGetUniqueId = function(prefix, separator)
{
	if ( this.ufnIsNull(prefix) ) prefix = "";
	if ( this.ufnIsNull(separator) ) {
		separator = 45;
	} else {
 ...
 * <--------------- 900 ------------------>	
 *		 
 * trace(this.ufnConvertXY(this,[0,0], Button00)); //output: [900,300]
 * //Button00을 기준으로 한 0,0 좌표는
 * //form 기준으로 했을 때 900, 300이 된다.
*/				
pForm.ufnConvertXY = function(XCompA, xy, XCompB)
{
	var tempX, tempY;
	var x, y;
	if ( XCompB )
	{
		...
		x = system.screenToClientX(XCompA, tempX) + this.ufnGetScrollLeft(XCompA);
		y = system.screenToClientY(XCompA, tempY) + this.ufnGetScrollTop(XCompA);				
	}
	else
	{
		x = xy[0];
 ...
 * @example
 * trace(this.ufnGetScrollLeft(Div01)); //output: 10
 * trace(this.ufnGetScrollLeft(Div01)); //output: 0
*/
pForm.ufnGetScrollLeft = function(XComp)
{
 ...
 * @example
 * trace(this.ufnGetScrollTop(Div01)); //output: 20
 * trace(this.ufnGetScrollTop(Div01)); //output: 0
 */
pForm.ufnGetScrollTop = function(XComp)
{
	if(XComp instanceof nexacro.Div){
		XComp = XComp.form;
	}
 ...
 * @example
 * var target = {};
 * this.ufnCopyProperties(target, {a: 1, b: "2", c: {"A": "3", "B": 4}});
 * for(var p in target)
 * {
 * 	trace(p + ":" + target[p]);
 *	// output : a:1
 ...
 * }
*/
pForm.ufnCopyProperties = function(tarobject, srcobject)
{
 ...
 * @example
 * // this = Form
 * trace(this.ufnGetSequenceId(this, "Button")); // output : Button0
 * trace(this.ufnGetSequenceId(this, "Button")); // output : Button1
 * // this = Form
 * trace(this.ufnGetSequenceId(this, "chk_")); // output : chk_0
 * trace(this.ufnGetSequenceId(this, "chk_")); // output : chk_1
*/		
pForm.ufnGetSequenceId = function(form, prefix)
{
	if ( this.ufnIsNull(form) ) 
	{
		trace("getSequenceId :: 1st argument doesn't exist !!");
		return null;
	}
	
	if ( this.ufnIsNull(prefix) ) 
	{
 ...
 * this.ufnEach(datas, function(prop, val, object) {
 * 	var result = "";
 * 	if ( !val )
 * 	{
 ...
 * }, this);
*/
pForm.ufnEach = function(object, func, scope)
{
	var p,
		scope = scope || object;
	for (p in object)
 ...
 * @example
 * trace(this.ufnGetTopLevelForm(this));	// output : [object Form]
*/		
pForm.ufnGetTopLevelForm = function(curForm)
{
	var p = curForm;
 ...
 * @example
 * var mon = ["Jan","Feb","Mar","Apr"];
 * var contain = this.ufnContains(mon, "Mar");
 * trace("contain==>" + contain);	// output : contain==>true
 * var contain = this.ufnContains(mon, "May");
 * trace("contain==>" + contain);	// output : contain==>false
*/
pForm.ufnContains = function(array, item, strict) 
{
	if (this.ufnIndexOf(array, item, null, strict) === -1) 
	{
		return false;
	}
	else
 ...
 * @example
 * var mon = ["Jan","Feb","Mar","Apr"];
 * var index = this.ufnIndexOf(mon, "Mar");
 * trace("index==>" + index);	// output : index==>2
 * var index = this.ufnIndexOf(mon, "May");
 * trace("index==>" + index);	// output : index==>-1
*/
pForm.ufnIndexOf = function(array, item, from, strict) 
{
 ...
 * @return N/A
 */   
pForm.ufnUpdateToDataset = function()
{
 ...
 * @return {boolean}
 */   
pForm.ufnDsIsUpdated = function (objDs)
{
 ...
 * @return {boolean}
 */   
pForm.ufnIsUpdatedRow = function (objDs, nRow)
{
	...
	else 
	{
		...
		{
			if (this.ufnIsUpdateColumn(objDs, nRow, i) == true) 
			{
				return true;
			}
		}
 ...
 * @return {boolean}
 */   
pForm.ufnIsUpdateColumn = function (objDs, nRow, Column)
{
	if (objDs.getRowType(nRow) == 2) 
	{
		if (this.ufnIsNull(objDs.getColumn(nRow, Column))) 
		{
			return false;
		}
	}
...
* @example 
* this.ufnPostSearch(obj);
*/
pForm.ufnPostSearch = function(obj)
{
	...
	// onusernotify 이벤트 추가
	objWeb.addEventHandler("onusernotify", this.ufnWebPost_onusernotify, this);	
};

/**
 * @description 우편번호 선택 시 콜백함수 호출
*/
pForm.ufnWebPost_onusernotify = function(obj, e)
{
 ...
 * @return {Boolean}
 */
pForm.ufnIsDate = function(strDate)
{
	if (this.ufnIsNull(strDate)) return false;
	
	var retVal;
	switch(strDate.length)
	{
		case 6://년월
			retVal =  this.ufnIsYM(strDate);
			break;
		case 8://년월일
			retVal =  this.ufnIsYMD(strDate);
			break;
		default:
			retVal = false; 
			break;
 ...
 * @return {Boolean}
 */
pForm.ufnIsYMD = function(strDate)
{
	var retVal = this.ufnGetDigit(strDate);
	...
	if (!this.ufnIsYM(strYM)) {
		return false;
	}
	var nDay   = Number(strDate.substr(6,2));	// 일자
	var nLastDay = Number(this.ufnGetLastDate(strYM).substr(6,2));//ufnGetLastDay에서 전체 20170331값이 넘어와서 .substr(6,2)추가 20170313
 ...
 * @return {Boolean}
 */
pForm.ufnIsYM = function(strDate)
{
var retVal = this.ufnGetDigit(strDate);
 ...
 * @return {Boolean}
 */
pForm.ufnIsTime = function(strTime)
{
 ...
 */
pForm.ufnGetLastDate = function(strDate)
{
	var s = "";
    if (strDate == null) {
 ...
 * @return {String} date
 */
pForm.ufnGetFirstDate = function(strDate)
{
    var s = "";

    if (strDate == null) {
		var objDate = new Date();
		s = objDate.getFullYear().toString() + (objDate.getMonth()+1).toString().padLeft(2, "0") + "01";     
    }
    else {
 ...
 */
pForm.ufnGetDay = function(strDate)
{
    var date = new Date();
 ...
 */
pForm.ufnGetDiffDate = function(sStartDate, sEndDate)
{
 ...
 */
pForm.ufnGetDiffMonth = function(sStartDate, sEndDate)
{
 ...
 */
pForm.ufnAddDate = function(strDate, nOffSet)
{
	var date = new Date();
 ...
 * @return {String} date
 */
pForm.ufnAddMonth = function(strDate, OffSet) 
{	
	 ...
	 */
	var ufnGetMonthLastDate = function(strMonth) {
		var iLastDay;
    ...
    if (strDate) {
        date = this.ufnStrToDate(strDate);
        d = (new Date(date)).addMonth(OffSet);
    } else {
    	date = this.ufnStrToDate(this.ufnGetDate());
		d = (new Date(date)).addMonth(OffSet);
    }
    
    if (ufnGetMonthLastDate(strDate) == date.getDate()) {
    	...
    	var ld = new Date((new Date(d)).getFullYear() 
    			, (new Date(d)).getMonth()
    			, ufnGetMonthLastDate(this.ufnDateToStr(new Date(d))));
 ...
 * @example : 
 */
pForm.ufnGetHolidays = function(nYear)
{
	var nYear;
	...
	/////// 음력 체크
	// 구정
	aHoliday[0] = this.ufnLunarToSolar((nYear-1) + "1230", false) + "설날";
	aHoliday[1] = this.ufnAddDate(aHoliday[0].substring(0, 8), 1) + "설날";
	aHoliday[2] = this.ufnAddDate(aHoliday[1].substring(0, 8), 1) + "설날";
	// 석가탄신일
	aHoliday[3] = this.ufnLunarToSolar(nYear + "0408", false) + "석가탄신일";
	// 추석
	aHoliday[4] = this.ufnLunarToSolar(nYear + "0814", false) + "추석";
	aHoliday[5] = this.ufnAddDate(aHoliday[4].substring(0, 8), 1) + "추석";
	aHoliday[6] = this.ufnAddDate(aHoliday[5].substring(0, 8), 1) + "추석";	
 ...
 * @example
 * var dt = this.ufnStr2Date("20130331");
 ...
 * trace(str); // output : 020130220
 */		
pForm.ufnSolarToLunar =  function(value) 
{
 ...
 */
pForm.ufnLunarToSolar = function(value, leapMonth)
{
 ...
 * @return {Date} date
 */
pForm.ufnStrToDate = function(inDate)
{
 ...
 */
pForm.ufnDateToStr = function(date)
{
 ...
 */
pForm.ufnGetWeek = function(strDate) 
{
	if (strDate.length != 8 || !this.ufnIsDigit(strDate)) {
		return "";
	}
 ...
 * @return {String} 8자리 날짜(YYYYMMMDD)문자열
 */
pForm.ufnGetBirthDateBySSN = function(ssn)
{
	var retVal;
 ...
 */
pForm.ufnGetBirthYear = function(sJuminNo)
{
 ...
 */
pForm.ufnGetAge = function(brtYmd, stdYmd)
{
	var retVal = -1;
	
	if (ufnIsEmpty(stdYmd)) {
		stdYmd = ufn_getDate();
	}

	if (ufn_isDate(brtYmd) && ufn_isDate(stdYmd)) {
